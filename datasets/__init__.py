...
from datasets.pretrain_dataset import XVLMPretrainDataset, BlipPretrainDataset, ClipPretrainDataset, ClipGPretrainDataset, Blip2PretrainDataset, LLaVAPretrainDataset, FlamingoPretrainDataset
...
from datasets.vqa_dataset import VQADataset , ScienceQADataset
...
def create_dataset(dataset, config, image_processor=None):
    ...
    if dataset == 'pretrain_xvlm':
        ...
        # if 'coco_file' in ood_config: ood_config.pop('coco_file')
        # if 'SkyScript_file' in ood_config: ood_config.pop('SkyScript_file')
        # if 'nwpu_file' in ood_config: ood_config.pop('nwpu_file')
        if 'rsitmd_file' in ood_config: ood_config.pop('rsitmd_file')
        ...
        if 'nwpu_file' in id_config: id_config.pop('nwpu_file')
        if 'coco_file' in id_config: id_config.pop('coco_file')
        # if 'vg_file' in id_config: id_config.pop('vg_file')
    ...
    elif dataset == 'pretrain_clip':
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Clip config파일 내 dataset정보 불러옴")
        general_dataset = ClipPretrainDataset(config, transform=pretrain_transform)
        region_dataset = ClipPretrainDataset(config, transform=pretrain_transform) # dummy op
        return general_dataset, region_dataset
    
    elif dataset == 'pretrain_clipG':
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Clip config파일 내 dataset정보 불러옴")
        general_dataset = ClipGPretrainDataset(config, transform=pretrain_transform)
        region_dataset = ClipGPretrainDataset(config, transform=pretrain_transform) # dummy op
        return general_dataset, region_dataset
    elif dataset == 'pretrain_blip2':
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Blip2 config파일 내 dataset정보 불러옴")
        general_dataset = Blip2PretrainDataset(config, transform=pretrain_transform)
        region_dataset = Blip2PretrainDataset(config, transform=pretrain_transform) # dummy op
        return general_dataset, region_dataset
    elif dataset == 'pretrain_llava' :
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Llava config파일 내 dataset정보 불러옴")
        general_dataset = LLaVAPretrainDataset(config, transform=pretrain_transform)
        region_dataset = LLaVAPretrainDataset(config, transform=pretrain_transform)
        return general_dataset, region_dataset
    elif dataset == 'pretrain_qwen_vl' :
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Qwen-VL config파일 내 dataset정보 불러옴")
        general_dataset = LLaVAPretrainDataset(config, transform=pretrain_transform)
        region_dataset = LLaVAPretrainDataset(config, transform=pretrain_transform)
        return general_dataset, region_dataset
    elif dataset == 'pretrain_flamingo':
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Pretrain Flamingo config파일 내 dataset정보 불러옴")
        general_dataset = FlamingoPretrainDataset(config, transform=pretrain_transform)
        region_dataset = FlamingoPretrainDataset(config, transform=pretrain_transform)
        return general_dataset, region_dataset
        ...
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : Retrieval config파일 내 dataset정보 불러옴")
        ...
        print("[Debug] datasets/_init.py__ -> create_dataset()함수 호출 : diffusion config파일 내 dataset정보 불러옴")
        from datasets.diffusion_dataset import DiffusionEvalDataset
        require_images = "FID" in set(config.get("metrics", []))
        eval_dataset = DiffusionEvalDataset(
            ann_file=config["prompts_file"],
            image_root=config.get("image_root",""),
            text_key=config.get("text_key","caption"),
            image_key=config.get("image_key","image"),
            mode=config.get("mode","per_image"),
            caption_policy=config.get("caption_policy","first"),
