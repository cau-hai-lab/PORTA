...
class RetrievalTrainDataset(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words=30):
        print("<retrieval_dataset.py : RetrievalTrainDataset클래스 init()함수 실행>")
        self.ann = []
        for f in ann_file:
    ...
    def __private_getitem__(self, index):
        #print("<retrieval_dataset.py : RetrievalTrainDataset클래스 -> private_getitem()함수 호출>")
        ann = self.ann[index]
    ...
    def __getitem__(self, index):
        #print("<retrieval_dataset.py :RetrievalTrainDataset클래스 -> getitem()함수 호출>")
...
class RetrievalEvalDataset(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words=30):
        print("<retrieval_dataset.py : RetrievalEvalDataset클래스 init()함수 실행>")
    ...
    def __private_getitem__(self, index):
        #print("<retrieval_dataset.py :RetrievalEvalDataset클래스 private_getitem()함수 호출>")
        ann = self.ann[index]
        image_path = os.path.join(self.image_root, ann['image'])
        image = Image.open(image_path).convert('RGB')
    ...
    def __getitem__(self, index):
        #print("<retrieval_dataset.py :RetrievalEvalDataset클래스 getitem()함수 호출>")
        # call the __private_getitem__ method, but make it
        # more robust to errors with try-catching, retries and logging
        for _ in range(10):
            try:
