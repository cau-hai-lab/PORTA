...
# === datasets/vision_datasets.py ===
# get_dataset 만 수정 (only_test 옵션 추가)

def get_dataset(dataset_name, data_dir, train_transform, test_transform, *, only_test=False, **kwargs):
    if dataset_name == "cifar100":
        # test 먼저 만들고, 클래스는 test 기준으로 통일
        test_dataset  = CIFAR100(data_dir, train=False, download=True, transform=test_transform)
        train_dataset = None if only_test else CIFAR100(data_dir, train=True,  download=True, transform=train_transform)
        classes = test_dataset.classes
        num_classes = len(classes)

    elif dataset_name == "cifar10":
        test_dataset  = CIFAR10(data_dir, train=False, download=True, transform=test_transform)
        train_dataset = None if only_test else CIFAR10(data_dir, train=True,  download=True, transform=train_transform)
        classes = test_dataset.classes
        num_classes = len(classes)

    elif dataset_name == "flowers102":
        test_dataset  = Flowers102(data_dir, split="test",  transform=test_transform)
        train_dataset = None if only_test else Flowers102(
            data_dir, split="train", transform=train_transform,
            merge_with_val=kwargs.get("merge_with_val", True)
        )
        classes = test_dataset.classes
        num_classes = len(classes)

    elif dataset_name == "imagenet":
        # ImageNet은 val만 사용 (train 필요 없으면 생성 안 함)
        test_dataset  = ImageNet(data_dir, split="val",   transform=test_transform)
        train_dataset = None if only_test else ImageNet(data_dir, split="train", transform=train_transform)
        classes = test_dataset.classes   # 사람이 읽는 이름으로 패치해두었다면 여기로 옴
        num_classes = len(classes)
