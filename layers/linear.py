...
#     if not getattr(module, "store_input_flag", False) and not getattr(module, "store_output_flag", False):
#         return
...
def memorization_hook(module, input_args, output):  #Flamingo용!!
    if not getattr(module, "store_input_flag", False) and not getattr(module, "store_output_flag", False):
        return
    ...
    # output 저장 (기존 유지)
    # -------------------------
    if getattr(module, "store_output_flag", False):
        if len(module.output_history) > 0:
            try:
                if torch.is_tensor(output) and torch.allclose(module.output_history[-1], output):
                    return
            except Exception:
                pass

        if getattr(module, "store_output_mem_save", False):
            module.output_history.append(output.to('cpu', non_blocking=True) if torch.is_tensor(output) else output)
        else:
            module.output_history.append(output)

    # -------------------------
    # input 저장 (🔥 None/비텐서 방어 추가)
    # -------------------------
    if getattr(module, "store_input_flag", False):
        if not input_args:
            return
        x = input_args[0]

        # ✅ None / 비텐서면 저장 스킵 (여기가 크래시 원인)
        if (x is None) or (not torch.is_tensor(x)):
            return

        # (선택) float 텐서만 저장하고 싶으면:
        # if not x.is_floating_point(): return

        if len(module.input_history) > 0:
            last = module.input_history[-1]
            if torch.is_tensor(last) and last.shape == x.shape:
                try:
                    if torch.allclose(last, x):
                        # print("Not registering the same input twice.")
                        return
                except Exception:
                    pass

        if getattr(module, "store_input_mem_save", False):
            module.input_history.append(x.detach().to('cpu', non_blocking=True))
        else:
            module.input_history.append(x.detach().to("cpu", dtype=torch.float16, non_blocking=True))
...
class Linear(nn.Linear):
    ...
    @staticmethod
    def from_pretrained(linear_layer: nn.Linear, mask_dtype=torch.bool, pattern_lock=False, mask_on_the_fly=False, store_input=False, store_output=False):
